...
public class RegisterPage extends FakeActivity implements OnClickListener,
	...
	public static final String URL = "http://kincai.ngrok.cc/shopkincai/app_api/appIsRegisteredPhone.php?phone=";
	...
	Handler handler2 = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case 0x2222:
				...
				if(code == 1) {//0表示已被注册
					Toast.makeText(activity, "此号码已注册过", Toast.LENGTH_SHORT).show();
				} else if (code == 0) {//没被注册
					...
					if (countryRules == null || countryRules.size() <= 0) {
						...
						String codeNum = tvCountryNum.getText().toString().trim();
						checkPhoneNum(phone, codeNum);
					}
	...
	public void onClick(View v) {
		...
		if (id == id_ll_back) {
			...
			//先判断是否被注册
			if(isNext) {//防止多次点击
				new Thread(new HttpGetThread(handler2, URL+etPhoneNum.getText().toString().trim(), "res")).start();
				isNext = false;
			}
