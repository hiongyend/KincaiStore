 ...
 * @author kincai
 * 
 * @description 首页商品的adapter(用的是自己写的listview)
 *
...
public class HomeProListAdapter extends BaseAdapter{
	...
	private static final String TAG = "HomeProListAdapter";
	private List<ProInfo> mDatas;
	...
	public HomeProListAdapter(Context context,
		...
		mDatas = (datas == null) ? new ArrayList<ProInfo>() : datas;
	...
	@Override
	public ProInfo getItem(int position) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(position < mDatas.size()) {
				holder.price.setText("¥ "
						+ mDatas.get(position).getiPrice());
			
			holder.title.setText(mDatas.get(position).getpName());
			holder.payNum.setText(new StringBuilder().append(mDatas.get(position).getPaynum()).append("人付款"));
		}
